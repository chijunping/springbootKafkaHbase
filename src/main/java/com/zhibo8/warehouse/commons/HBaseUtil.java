...
//    #################################### 连接池操作 #############################################################
    ...
    public static void init(int initialSize, int maxPoolSize_, long maxWaitTime_, String zkHost) { //maxActive 允许最大并发数（最大连接数）、maxWait 允许最大等待时间
        ...
        if (conf == null) {
            ...
            conf.set(HConstants.HBASE_RPC_TIMEOUT_KEY, "60000");
            conf.set(HConstants.HBASE_META_SCANNER_CACHING, "1000");
            conf.set(HConstants.HBASE_CLIENT_SCANNER_CACHING, "1000");
        }
        ...
        for (int i = 0; i < initialSize; i++) {
            ...
            logger.info("busyPoolSize=" + busy.size() + ",idlePoolSize=" + idle.size() + ",sumSize=" + (busy.size() + idle.size()) + ",currPoolSize=" + currPoolSize);
        }
    }
    ...
    public static Connection getConnection() throws Exception {
        ...
        while (null == conn) {
            ...
            if (conn != null) {
                busy.offer(conn);
            } else if (currPoolSize.incrementAndGet() <= maxPoolSize) {//3.如果空闲队列无资源，且当前连接池大小<maxPoolSize,则手动创建新资源，并放入busy队列
                //创建 Connection 连接
                conn = createConnection();
                //存入busy队列
                busy.offer(conn);
            } else if (currPoolSize.incrementAndGet() > maxPoolSize) {
            ...
            logger.info("busyPoolSize=" + busy.size() + ",idlePoolSize=" + idle.size() + ",sumSize=" + (busy.size() + idle.size()) + ",currPoolSize=" + currPoolSize);
            //获取超时处理
            if ((System.currentTimeMillis() - borrowStartTime) > maxWaitTime) {
                throw new Exception("获取Hbase connection 超时...");
            }
        }
        return conn;
    }
...
//    ##################################### 数据操作 ##############################################
    ...
    public static boolean insertRows(String tableName, String family, List<List<Object>> rows, long writeBufferSize) {
        ...
        List<Put> puts = rows2Puts(tableName, family, rows);
        try {
            conn = getConnection();
            admin = getHBaseAdmin(conn);
            if (admin.tableExists(TableName.valueOf(tableName))) {
            ...
            logger.error(e.getMessage() + "\n" + "resordSize=" + puts.size() + ",spendTime=" + (System.currentTimeMillis() - startTime), e);
     ...
     * @throws IOException
     */
    public static long insertRowsByMutator(String tableName, String family, List<List<Object>> rows, long writeBufferSize) throws Exception {
        ...
        final BufferedMutator.ExceptionListener listener = new BufferedMutator.ExceptionListener() {
            ...
            public void onException(RetriesExhaustedWithDetailsException e, BufferedMutator mutator) {
                for (int i = 0; i < e.getNumExceptions(); i++) {
                    System.out.printf("Failed to sent put ", e.getRow(i) + ".");
    ...
    public static List<Put> rows2Puts(String tableName, String family, List<List<Object>> rows) {
